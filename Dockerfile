...
RUN --mount=type=cache,target=/cache find /root/.m2/repository -type f -name "*.jar" -o -name "*.pom" -o -name "*.slingosgifeature" -o -name "*.gz"> /tmp/archives && tar cfvz /cache/files.tar.gz -T /tmp/archives && rm -rf /root/.m2/repository
